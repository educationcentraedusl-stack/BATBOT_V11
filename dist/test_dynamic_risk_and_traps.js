...
async function runDynamicRiskAndTrapTests() {
    ...
    const riskGuard = new risk_1.RiskGuard({ maxPositionSizeUsdt: 5000, minCooldownMs: 0 });
    const execClient = new binance_1.BinanceExecutionClient({ apiKey: "MOCK_KEY", apiSecret: "MOCK_SECRET", useTestnet: true });
    const strategy = new engine_1.StrategyEngine(client, riskGuard, execClient, {
        symbol: "BTCUSDT",
        orderQuantity: 0.001,
        minAiConfidence: 0.6,
    });
    ...
    if (tickResult2.riskResult) {
        ...
        console.assert(tickResult2.riskResult.reasonCode === "REJECTED_TOXIC_FLOW" ||
            tickResult2.riskResult.reasonCode === "REJECTED_COUNTER_TREND_REGIME", "Expected REJECTED_TOXIC_FLOW or REJECTED_COUNTER_TREND_REGIME");
    }
    ...
    if (tickResult3.riskResult) {
        ...
        console.assert(tickResult3.riskResult.reasonCode === "REJECTED_LIQUIDITY_SWEEP_TRAP", "Expected REJECTED_LIQUIDITY_SWEEP_TRAP");
    }
