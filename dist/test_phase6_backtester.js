...
const multiAssetBacktester_1 = require("./backtest/multiAssetBacktester");
...
async function runPhase6BacktesterQaSuite() {
    console.log("=========================================================================");
    ...
    const engine = new multiAssetBacktester_1.MultiAssetBacktestEngine(sab, {
    ...
    for (let i = 0; i < totalTicks; i++) {
        ...
        if (cycle % 12 === 2)
            aiDirection = 0.65; // Long signal
        if (cycle % 12 === 8)
            aiDirection = -0.65; // Short/Exit signal
        ticks.push({
            assetIdx,
            symbol,
            timestamp: now + i * 100,
    ...
    const result = engine.runTicks(ticks);
    ...
    assert(result.totalTradesExecuted === result.winningTrades + result.losingTrades, `Completed round-trip trade tally arithmetic mismatch: ${result.totalTradesExecuted} !== ${result.winningTrades} + ${result.losingTrades}`);
    ...
    const csvLines = ["assetIdx,symbol,timestamp,bidPrice,askPrice,obi,cvd,hawkes,volatility,aiDirection"];
    for (let i = 0; i < 600; i++) {
        const assetIdx = i % maxAssets;
    ...
    const csvStream = stream_1.Readable.from(csvLines.join("\n"));
    ...
    assert(streamResult.totalTradesExecuted === streamResult.winningTrades + streamResult.losingTrades, "Stream trade arithmetic mismatch");
    ...
    const tearSheet = multiAssetBacktester_1.MultiAssetBacktestEngine.generateTearSheet(result);
