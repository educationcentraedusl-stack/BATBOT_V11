...
class AutoRecalibrationManager {
    ...
    getSabBufferForNapi() {
        if (!this.client) {
            ...
            this.client = new marketDataClient_1.MarketDataClient(sab, maxAssets, slotsPerAsset);
        }
     ...
     */
    async runTkanTrainingPipeline(force = false) {
    ...
    async runRecalibrationPipeline(currentIc, force = false) {
        if (this.isRecalibrating) {
            return false;
        }
        ...
        if (!force && currentIc >= 0.0500) {
            console.log(`[BATBOT_V11][ALPHA_IMMUNITY] High Spearman IC (+${currentIc.toFixed(4)} >= +0.0500). Predictive power is optimal; recalibration rejected.`);
            this.driftTickCounter = 0;
            return false;
        }
