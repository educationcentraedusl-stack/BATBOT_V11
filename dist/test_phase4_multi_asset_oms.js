...
const multi_asset_executor_1 = require("./execution/multi_asset_executor");
...
async function runPhase4MultiAssetOmsTest() {
    ...
    const executor = new multi_asset_executor_1.MultiAssetExecutor({
    ...
    for (let idx = 0; idx < activeSymbols.length; idx++) {
        ...
        const res = executor.submitIntent(symbol, "BUY", "LIMIT", 1.0, midPrice * 0.999, midPrice, 50000.0, 0.001, 0.01, 1.0, 0.20);
    ...
    const leverageCapRes = executor.submitIntent("SOLUSDT", "BUY", "LIMIT", 1.0, 150.0, 150.0, 50000.0, 0.001, 0.01, 3.5, 0.20);
    ...
    const correlationRes = executor.submitIntent("BNBUSDT", "BUY", "LIMIT", 1.0, 500.0, 500.0, 50000.0, 0.001, 0.01, 1.0, 0.92);
    ...
    const largeOrderRes = executor.submitIntent("XRPUSDT", "BUY", "LIMIT", 50000.0, 0.50, 0.50, 10000.0, 0.1, 0.0001, 1.0, 0.20);
    ...
    for (let i = 0; i < ITERATIONS; i++) {
        ...
        executor.submitIntent(symbol, i % 2 === 0 ? "BUY" : "SELL", "LIMIT", 1.0, basePrice, basePrice, 50000.0, 0.001, 0.01, 0.5, 0.10);
        if (i % 100 === 0) {
            executor.popNextPacket();
        }
    }
    const elapsedMs = perf_hooks_1.performance.now() - startTime;
    ...
    console.log("================================================================================");
    assert(latencyPerTickUs < 5.0, "Latency target < 5.0 us violated: " + latencyPerTickUs.toFixed(3) + " us");
    assert(throughput > 150000, "Throughput target > 150,000 intents/sec violated: " + throughput.toLocaleString() + " intents/sec");
