...
class StrategyEngine {
    ...
    async reconcileFlatPositionWithUserTrades(posSide, delayMs = 250) {
        ...
        const settleAction = async () => {
            ...
            if (needsLongSettle) {
                ...
                const longExitTrades = trades.filter((t) => (t.positionSide === "LONG" || (t.positionSide === "BOTH" && !t.buyer) || (t.side === "SELL" && t.positionSide !== "SHORT")) && parseFloat(t.qty || "0") > 0);
            ...
            if (needsShortSettle) {
                ...
                const shortExitTrades = trades.filter((t) => (t.positionSide === "SHORT" || (t.positionSide === "BOTH" && t.buyer) || (t.side === "BUY" && t.positionSide !== "LONG")) && parseFloat(t.qty || "0") > 0);
                let exactPnl = undefined;
                let exactCommission = undefined;
                let exactExitPrice = undefined;
    ...
    async dispatchExchangeStopLossOrder(slotId, entryPrice, quantity, side, stopLossPrice) {
        ...
        try {
            console.log(`[EXCHANGE_SL_ENGINE][DISPATCHING] [${this.config.symbol}:${slotId}] Submitting resting STOP_MARKET order on Binance: ${exitSide} ${formattedQty} @ stopPrice $${formattedSlPx} (ClId: ${clientOrderId})...`);
