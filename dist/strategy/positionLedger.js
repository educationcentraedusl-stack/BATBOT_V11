...
class HedgePositionLedger {
    ...
    occupyCoreLong(quantity, entryPrice, tpPercent, slPercent) {
        ...
        if (this.coreLong.isOccupied && this.coreLong.quantity > 0) {
            ...
            this.coreLong.stopLossPrice = symbolPrecision_1.SymbolPrecisionRegistry.formatPrice(this.symbol, newAvgEntry * (1 - slPercent / 100));
    ...
    occupyShortSlot(slotIndex, quantity, entryPrice, tpPercent, slPercent) {
        ...
        if (slot.isOccupied && slot.quantity > 0) {
            ...
            slot.stopLossPrice = symbolPrecision_1.SymbolPrecisionRegistry.formatPrice(this.symbol, newAvgEntry * (1 + slPercent / 100));
    ...
    evalSingleSlotSota(slot, markPrice, stoikovMicroPrice, hazardMetrics, hjbEngine, volMetrics, nowMs, halfLifeSec, roundTripFeeBuffer, minNetAlpha, collarDistancePct) {
        ...
        // -------------------------------------------------------------------------
        let msSopcStopPrice = 0;
        if (isLong) {
            // Anchored to Stoikov Micro-Price Peak
            slot.peakPrice = Math.max(slot.peakPrice || slot.entryPrice, stoikovMicroPrice);
            const collarOffsetUsdt = slot.peakPrice * collarDistancePct;
            msSopcStopPrice = this.formatPriceFast(slot.peakPrice - collarOffsetUsdt);
            // Ratchet up only
            ...
            // Anchored to Stoikov Micro-Price Trough
            slot.troughPrice = (slot.troughPrice && slot.troughPrice > 0)
                ? Math.min(slot.troughPrice, stoikovMicroPrice)
                : stoikovMicroPrice;
            const collarOffsetUsdt = slot.troughPrice * collarDistancePct;
            msSopcStopPrice = this.formatPriceFast(slot.troughPrice + collarOffsetUsdt);
            // Ratchet down only
        ...
        if (slot.breakEvenLocked && slot.breakEvenPrice && slot.breakEvenPrice > 0) {
            msSopcStopPrice = isLong
                ? Math.max(msSopcStopPrice, slot.breakEvenPrice)
                : Math.min(msSopcStopPrice, slot.breakEvenPrice);
        }
        // Check Trigger
