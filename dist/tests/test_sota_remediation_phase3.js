...
function assertNear(actual, expected, tol, label) {
    ...
    if (ok) {
        console.log(`  PASS: ${label} [actual=${actual.toFixed(6)}]`);
        passed++;
    }
    else {
        console.error(`  FAIL: ${label} [actual=${actual.toFixed(6)}, expected=${expected.toFixed(6)}]`);
        failed++;
    }
}
...
{
    const engine = new microstructureHazardEngine_1.MicrostructureHazardEngine("BTCUSDT");
...
{
    const v = new volatilitySurfaceEngine_1.VolatilitySurfaceEngine("BTC", 600);
...
if (failed > 0) {
    ...
    console.log(`\nPASS: All ${passed} tests passed. Defects 1-10 VERIFIED.`);
    process.exit(0);
}
