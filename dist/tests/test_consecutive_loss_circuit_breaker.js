...
async function runConsecutiveLossCircuitBreakerTestSuite() {
    ...
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(0) === 0, "0 losses cooldown should be 0ms");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(1) === 15_000, "1 loss cooldown should be 15,000ms (15s)");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(2) === 60_000, "2 losses cooldown should be 60,000ms (60s)");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(3) === 180_000, "3 losses cooldown should be 180,000ms (180s)");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(4) === 180_000, "4 losses cooldown should be 180,000ms (180s)");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(5) === 900_000, "5 losses cooldown should be 900,000ms (15m hard halt)");
    assert(risk_1.RiskGuard.calculateExponentialLossCooldownMs(10) === 900_000, ">= 5 losses cooldown capped at 900,000ms");
    ...
    const multiRiskGuard = new risk_1.MultiAssetRiskGuard();
