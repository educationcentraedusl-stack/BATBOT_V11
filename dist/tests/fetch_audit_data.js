...
async function main() {
    ...
    try {
        const income = await client.request("GET", "/fapi/v1/income", { limit: 30 }, true);
    ...
    for (const sym of symbols) {
        try {
            const trades = await client.request("GET", "/fapi/v1/userTrades", { symbol: sym, limit: 20 }, true);
    ...
    for (const sym of symbols) {
        try {
            const orders = await client.request("GET", "/fapi/v1/allOrders", { symbol: sym, limit: 15 }, true);
