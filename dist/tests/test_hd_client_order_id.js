...
for (const sym of symbols) {
    for (const slot of slots) {
        for (const ot of orderTypes) {
            const cid = clientOrderIdGenerator_1.ClientOrderIdGenerator.generate(sym, slot, ot);
            sampleCids.push(cid);
...
console.log(`  - BTC Long Entry:     ${clientOrderIdGenerator_1.ClientOrderIdGenerator.generate("BTCUSDT", "CORE_LONG", "EN")}`);
console.log(`  - XRP Long SL:        ${clientOrderIdGenerator_1.ClientOrderIdGenerator.generate("XRPUSDT", "CORE_LONG", "SL")}`);
console.log(`  - DOGE Short TP1:     ${clientOrderIdGenerator_1.ClientOrderIdGenerator.generate("DOGEUSDT", "SHORT_SLOT_0", "TP1")}`);
console.log(`  - AVAX Emergency Out: ${clientOrderIdGenerator_1.ClientOrderIdGenerator.generate("AVAXUSDT", "SHORT_SLOT_1", "EM")}`);
// -----------------------------------------------------------------------------------------
...
for (const tc of testCases) {
    const generatedCid = clientOrderIdGenerator_1.ClientOrderIdGenerator.generate(tc.sym, tc.slot, tc.type);
    const parsed = clientOrderIdGenerator_1.ClientOrderIdGenerator.parse(generatedCid);
    if (!parsed) {
...
for (let i = 0; i < 4096; i++) {
    const cid = clientOrderIdGenerator_1.ClientOrderIdGenerator.generate("XRPUSDT", "CORE_LONG", "TP1");
    if (seenCids.has(cid)) {
        collisions++;
    }
    seenCids.add(cid);
...
const ledger = new positionLedger_1.HedgePositionLedger("XRPUSDT");
