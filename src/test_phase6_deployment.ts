...
async function httpGet(urlStr: string): Promise<{ status: number; body: string }> {
...
async function runPhase6DeploymentTests() {
  ...
  const testnetClient = new BinanceExecutionClient();
  ...
  totalTests++;
  const liveClient = new BinanceExecutionClient({ useTestnet: false });
