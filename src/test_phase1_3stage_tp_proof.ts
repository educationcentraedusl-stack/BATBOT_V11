...
async function runPhase1Proof() {
  ...
  for (const asset of testAssets) {
    ...
    const ledger = new HedgePositionLedger(asset.symbol);
    ...
    intents.forEach((intent, idx) => {
      ...
      console.log(
        `  📌 Stage #${idx + 1} TP Order -> Side: ${intent.side} | Type: ${intent.type} | Qty: ${intent.quantity} ${asset.symbol} | Target Price: $${intent.price} | Notional: $${notional.toFixed(2)} USDT | TIF: ${intent.timeInForce} (POST_ONLY)`
      );
    });
