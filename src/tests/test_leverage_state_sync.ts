...
import { MultiAssetPositionLedger } from "../strategy/positionLedger";
...
async function runLeverageStateSyncTest() {
  ...
  const writeFloat = (assetIdx: number, slot: number, val: number) => {
    bitcastFloat[0] = val;
  ...
  const positionLedger = new MultiAssetPositionLedger(symbols, 100_000.0);
  ...
  const mockExchangePositions: BinancePositionRisk[] = [
    {
      symbol: "BTCUSDT",
      positionAmt: "0.0008",
      entryPrice: "63166.80",
  ...
  const mockOpenOrders: BinanceOrderResponse[] = [];
  ...
  if (!bnbEngine) throw new Error("BNBUSDT engine must exist");
  ...
  if (!solEngine) throw new Error("SOLUSDT engine must exist");
