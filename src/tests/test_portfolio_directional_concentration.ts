...
function createMockPositionRisk(
  symbol: string,
  amt: string,
  entryPx: string,
  posSide: string,
...
async function runPortfolioDirectionalConcentrationTestSuite(): Promise<void> {
  console.log("================================================================================");
  ...
  const engineBTC = new StrategyEngine(client, riskGuard, execClient, {
    symbol: "BTCUSDT",
    assetIndex: 0,
    orderQuantity: 0.001,
    cooldownMs: 0,
  ...
  const engineETH = new StrategyEngine(client, riskGuard, execClient, {
    symbol: "ETHUSDT",
    assetIndex: 1,
    orderQuantity: 0.05,
    cooldownMs: 0,
  ...
  const engineSOL = new StrategyEngine(client, riskGuard, execClient, {
    symbol: "SOLUSDT",
    assetIndex: 2,
    orderQuantity: 1.0,
    cooldownMs: 0,
  ...
  const engineAVAX = new StrategyEngine(client, riskGuard, execClient, {
    symbol: "AVAXUSDT",
    assetIndex: 3,
    orderQuantity: 5.0,
    cooldownMs: 0,
  ...
  for (let aIdx = 0; aIdx < 4; aIdx++) {
    ...
    client.writeAtomicFloat64Asset(aIdx, SAB_SLOTS.SPREAD_VELOCITY, 0.0);
  ...
  client.writeAtomicFloat64Asset(3, SAB_SLOTS.AI_DIRECTION, 0.85);
  client.writeAtomicFloat64Asset(3, SAB_SLOTS.AI_CONFIDENCE, 0.90);
  client.setOBI(0.50, 3);
  client.setCVD(50.0, 3);
  const sig1 = engineAVAX.evaluateTick();
  if (sig1.executionPromise) await sig1.executionPromise;

  // Tick 2: Surging buy pressure on AVAX
  await new Promise((r) => setTimeout(r, 10));
  const nowMs2 = timeSynchronizer.getAdjustedNowMs();
  ...
  assert(
    blockedSignal.riskResult?.reasonCode === "PORTFOLIO_CONCENTRATION_LIMIT",
    `Expected reasonCode "PORTFOLIO_CONCENTRATION_LIMIT", got "${blockedSignal.riskResult?.reasonCode}"`
  );
  ...
  await new Promise((r) => setTimeout(r, 10));
  const nowMs3 = timeSynchronizer.getAdjustedNowMs();
  ...
  if (approvedSignal.executionPromise) await approvedSignal.executionPromise;
  ...
  if (baseShortSig.executionPromise) await baseShortSig.executionPromise;

  await new Promise((r) => setTimeout(r, 10));
  const nowMs5 = timeSynchronizer.getAdjustedNowMs();
  ...
  assert(
    blockedShortSignal.riskResult?.reasonCode === "PORTFOLIO_CONCENTRATION_LIMIT",
    `Expected reasonCode "PORTFOLIO_CONCENTRATION_LIMIT", got "${blockedShortSignal.riskResult?.reasonCode}"`
  );
  ...
  await new Promise((r) => setTimeout(r, 10));
  const nowMs6 = timeSynchronizer.getAdjustedNowMs();
  ...
  if (approvedShortSignal.executionPromise) await approvedShortSignal.executionPromise;
