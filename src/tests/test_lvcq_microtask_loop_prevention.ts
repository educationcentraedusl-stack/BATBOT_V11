...
async function runLvcqMicrotaskLoopTest() {
  ...
  mockExecClient.placePositionStopLoss = async (
    symbol: string,
    side: "BUY" | "SELL",
  ...
  mockExecClient.cancelOrder = async (
    symbol: string,
  ...
  const engine = new StrategyEngine(client, riskGuard, mockExecClient, { symbol: "BTCUSDT", assetIndex: 0 }, undefined, ledger);
