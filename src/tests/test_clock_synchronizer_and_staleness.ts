import { TimeSynchronizer, timeSynchronizer } from "../utils/timeSynchronizer";
...
function storeAtomicFloat(bigIntView: BigInt64Array, slot: number, value: number): void {
...
async function runClockSyncAndStalenessTests() {
  ...
  const customSync = new TimeSynchronizer({
  ...
  timeSynchronizer.setManualOffsetMs(-2390);

  const riskGuard = new RiskGuard();
  const mockExecClient = new BinanceExecutionClient({
    apiKey: "test_key",
    apiSecret: "test_secret",
    useTestnet: true,
  });

  mockExecClient.placeOrder = async (params): Promise<BinanceOrderResponse> => {
    return {
      orderId: 888111,
      ...
      updateTime: timeSynchronizer.getAdjustedNowMs(),
    };
  };
  ...
  const exchangeServerTimeMs = timeSynchronizer.getAdjustedNowMs() - 15;
  ...
  const expectedLockTime = timeSynchronizer.getAdjustedNowMs() + 5000;
  ...
  for (let i = 2; i <= 1000; i++) {
    ...
    const freshExchangeTs = BigInt(timeSynchronizer.getAdjustedNowMs() - jitterMs) * 1000000n;
  ...
  const expectedTimestamp = timeSynchronizer.getAdjustedNowMs();
