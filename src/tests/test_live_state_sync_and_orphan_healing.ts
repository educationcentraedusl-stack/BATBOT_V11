...
import { OrderTradeUpdatePayload, AccountUpdatePayload } from "../execution/userDataStream";

async function runStateSyncAndOrphanHealingTest(): Promise<void> {
  console.log("=========================================================================");
  ...
  if (!solEngine) throw new Error("SOLUSDT engine not found");

  // Simulate incoming WebSocket trade fill for SOLUSDT Short Entry
  const mockWsOrderUpdate: OrderTradeUpdatePayload = {
  ...
  const mockAccountUpdate: AccountUpdatePayload = {
