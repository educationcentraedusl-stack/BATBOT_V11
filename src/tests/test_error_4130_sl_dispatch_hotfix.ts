...
async function runHotfixTests(): Promise<void> {
  console.log("=========================================================================");
  console.log("  TEST: ERROR -4130 SL DISPATCH DIRECTION & AUTO-RECOVERY VERIFICATION  ");
  console.log("=========================================================================\n");

  SymbolPrecisionRegistry.preseedOfflineDefaults(["XRPUSDT", "ETHUSDT", "BTCUSDT"]);
  ...
  assert.strictEqual(shortSlRes.orderId, 987654321, "Short SL orderId must be returned");
  const shortPayload = capturedPayloads[0];
  assert.strictEqual(shortPayload.symbol, "XRPUSDT", "Symbol must match");
  assert.strictEqual(shortPayload.side, "BUY", "Short position exit side MUST be BUY");
  assert.strictEqual(shortPayload.positionSide, "SHORT", "PositionSide MUST be SHORT");
  assert.strictEqual(shortPayload.type, "STOP_MARKET", "Order type MUST be STOP_MARKET");
  assert.strictEqual(shortPayload.closePosition, "true", "closePosition MUST be string 'true'");
  assert.strictEqual(shortPayload.quantity, undefined, "quantity MUST be undefined when closePosition is true");
  assert.strictEqual(shortPayload.reduceOnly, undefined, "reduceOnly MUST be undefined when closePosition is true");
  assert.strictEqual(shortPayload.price, undefined, "price MUST be undefined for STOP_MARKET");
  assert.strictEqual(shortPayload.timeInForce, undefined, "timeInForce MUST be undefined for STOP_MARKET");
  ...
  assert.strictEqual(correctedPayload.side, "BUY", "Misaligned side 'SELL' on SHORT position MUST be corrected to 'BUY'");
  assert.strictEqual(correctedPayload.positionSide, "SHORT", "PositionSide MUST be SHORT");
  ...
  assert.strictEqual(longPayload.symbol, "ETHUSDT", "Symbol must match");
  assert.strictEqual(longPayload.side, "SELL", "Long position exit side MUST be SELL");
  assert.strictEqual(longPayload.positionSide, "LONG", "PositionSide MUST be LONG");
  assert.strictEqual(longPayload.closePosition, "true", "closePosition MUST be 'true'");
  ...
  const hedgeLedger = new HedgePositionLedger("XRPUSDT");
  ...
  assert.strictEqual(enginePayload.positionSide, "SHORT", "Engine SHORT SL MUST pass positionSide: SHORT");
  ...
  (recoveryClient as any).request = async (_method: string, path: string, payload?: any): Promise<any> => {
  ...
  assert.strictEqual(attemptCount, 2, "placeOrder must have retried after catching -4130");
  assert.strictEqual(cancelledOrderIds.length, 1, "Must have cancelled 1 conflicting order");
  assert.strictEqual(cancelledOrderIds[0], 111222, "Must have cancelled the conflicting stale SL order");
  assert.strictEqual(recoveredRes.orderId, 999888777, "Recovered order ID must be returned");
