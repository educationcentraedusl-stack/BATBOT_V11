...
function runHjbBreathingTest() {
  ...
  const hjbEngine = new HJBReservationEngine("BTCUSDT", 0.10, 60.0, 1.5);
  ...
  const evalImmediate = hjbEngine.getOptimalExitBoundary(
    "LONG",
    entryPrice,
    normalNoisePrice,
    0.001,
  ...
  const evalCatastrophic = hjbEngine.getOptimalExitBoundary(
    "LONG",
    entryPrice,
    catastrophicPrice,
    0.001,
