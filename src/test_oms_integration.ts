...
async function main() {
  ...
  const coreInit = initCore();
  console.log(`Core Initialization: ${coreInit}`);

  // 2. Initialize OMS Engine
  const omsStarted = startOmsEngine("BTCUSDT", 100000.0, "", "", true);
  ...
  // 4. Evaluate OMS Tick
  const intentJson = evaluateOmsTick(Buffer.from(sab));
  console.log(`Evaluated Intent JSON: ${intentJson}`);

  // 5. Query OMS Metrics & Position Snapshot
  const metricsJson = getOmsMetrics();
  console.log(`OMS Metrics: ${metricsJson}`);

  const positionJson = getPositionSnapshot();
