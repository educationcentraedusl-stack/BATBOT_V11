                            ...
                            if let Some(metrics) = orchestrator.lob_manager.get_metrics_for_asset(asset_idx) {
                                ...
                                if let Some(ref ai) = orchestrator.ai_engine {
                                    ...
                                    if confidence >= 0.65 && signal_direction.abs() > 0.5 {
                                        ...
                                        let price = if signal_direction > 0.0 {
                                            metrics.last_spread + 100.0
                                        } else {
                                            100.0
                                        };
                                        ...
                                        if let Some(ref risk) = orchestrator.risk_guard {
                                            let current_drawdown = 0.01;
                                            let current_exposure = 500.0;
                                        ...
                                        if risk_passed {
                                            ...
                                            let sym_bytes = format!("ASSET_{}", asset_idx).into_bytes();
                                            let sym_len = sym_bytes.len().min(16);
                                            symbol_buf[..sym_len].copy_from_slice(&sym_bytes[..sym_len]);
