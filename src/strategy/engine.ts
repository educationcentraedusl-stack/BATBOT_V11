...
export class StrategyEngine {
  ...
  private lastRiskAuditTimestamp: number = 0;
  private static readonly RISK_AUDIT_CADENCE_MS: number = 1000;
  private lastLongSlRatchetTimestamp: number = 0;
  private lastShortSlRatchetTimestamp: number = 0;
  private static readonly SL_RATCHET_MIN_INTERVAL_MS: number = 1000;
  ...
  private rollingPacketAgeMs: number = 1000;
  private rollingPacketJitterMs: number = 200;
  private readonly stalenessMinFloorMs: number = 2500;
  private readonly stalenessMaxCapMs: number = 6000;
  private readonly stalenessJitterMultiplier: number = 4.0;
  private readonly packetAgeAlpha: number = 0.05;
  private readonly packetJitterBeta: number = 0.10;
  ...
  public hasPendingEntryForSide(side: "LONG" | "SHORT"): boolean {
    for (const pending of this.pendingEntryOrders.values()) {
  ...
  public hasAnyPendingEntry(): boolean {
   ...
   */
  public annihilateRestingEntryOrders(reason: string): number {
    const count = this.pendingEntryOrders.size;
    if (count === 0) return 0;

    console.warn(
      `[AROS_CA][SPREAD_BLOWOUT_ANNIHILATION] [${this.config.symbol}] ${reason}! Annihilating ${count} resting/pending entry order(s) to prevent adverse selection fill...`
    );
    ...
    for (const [orderKey, pending] of this.pendingEntryOrders.entries()) {
      ...
      if (orderKey > 0) {
        ...
        this.executionClient.cancelOrder(this.config.symbol, orderKey).catch((err: unknown) => {
  ...
  public async reconcileFlatPositionWithUserTrades(
    ...
    const settleAction = async () => {
      ...
      const effectiveLongOpenTime = longOpenTime > 0 ? longOpenTime : (timeSynchronizer.getAdjustedNowMs() - fallbackWindowMs);
      const effectiveShortOpenTime = shortOpenTime > 0 ? shortOpenTime : (timeSynchronizer.getAdjustedNowMs() - fallbackWindowMs);
      ...
      if (order.orderType === "LIMIT" || order.isMaker || (parsedCid && parsedCid.orderType.startsWith("TP"))) {
        ...
        if (targetSlotId) {
          ...
          if (!sideSummary.isOccupied || sideSummary.totalQuantity <= 0) {
            ...
            if (slId) {
              this.executionClient.cancelOrder(this.config.symbol, slId).catch((err: unknown) => {
            ...
            this.syncExchangeStopLossOrder(posSide, sideSummary.totalQuantity, res.newStopLossPrice).catch((err: unknown) => {
      ...
      if (execPx > 0 && execQty > 0) {
        if (isEntrySide) {
          ...
          if (targetPosSide === "LONG") {
            ...
            this.syncExchangeStopLossOrder("LONG", aggLong.totalQuantity, aggLong.stopLossPrice).catch((err: unknown) => {
            ...
            this.syncExchangeStopLossOrder("SHORT", aggShort.totalQuantity, aggShort.stopLossPrice).catch((err: unknown) => {
  ...
  public async dispatchExchangeStopLossOrder(
    ...
    const clientOrderId = ClientOrderIdGenerator.generate(this.config.symbol, side === "LONG" ? "CORE_LONG" : "SHORT_AGG", "SL");
    try {
      console.log(
        `[EXCHANGE_SL_ENGINE][DISPATCHING] [${this.config.symbol}:${side}] Submitting sovereign position-level STOP_MARKET order on Binance: ${exitSide} (positionSide: ${positionSide}, closePosition: true) @ stopPrice $${formattedSlPx} (ClId: ${clientOrderId})...`
      );
  ...
  public async syncExchangeStopLossOrder(
    ...
    if (existingLock) {
      ...
      if (lockAge > StrategyEngine.MAX_SL_LOCK_HOLD_MS) {
        ...
        console.warn(
          `[EXCHANGE_SL_ENGINE][LOCK_EVICTED] [${this.config.symbol}:${side}] Stale SL sync lock detected (Age: ${lockAge}ms > ${StrategyEngine.MAX_SL_LOCK_HOLD_MS}ms, Epoch: #${existingLock.epoch}). Auto-evicting zombie lock and aborting stalled operation...`
        );
        try {
        ...
        console.log(
          `[EXCHANGE_SL_ENGINE][LOCKED] [${this.config.symbol}:${side}] Sync is already in-flight (Epoch: #${existingLock.epoch}, Age: ${lockAge}ms). Coalesced latest target SL $${newStopLossPrice}.`
        );
        return;
      }
    }
    ...
    try {
      ...
      while (drainIterations < maxDrainIterations) {
        ...
        if (this.slSyncLocks.get(lockKey)?.epoch !== currentEpoch) {
          console.warn(
            `[EXCHANGE_SL_ENGINE][EPOCH_FENCED] [${this.config.symbol}:${side}] Worker Epoch #${currentEpoch} was evicted (Active Epoch: #${this.slSyncLocks.get(lockKey)?.epoch}). Terminating stale worker.`
          );
          return;
        }
        ...
        if (this.slSyncLocks.get(lockKey)?.epoch !== currentEpoch) {
          console.warn(
            `[EXCHANGE_SL_ENGINE][EPOCH_FENCED] [${this.config.symbol}:${side}] Worker Epoch #${currentEpoch} evicted after cancelOrder. Aborting.`
          );
          return;
        }
        ...
        const placedOrderId = await this.dispatchExchangeStopLossOrder(
          side,
          recheckSummary.vwapEntryPrice,
          recheckSummary.totalQuantity,
          currentTargetPrice,
        ...
        if (this.slSyncLocks.get(lockKey)?.epoch !== currentEpoch) {
          console.warn(
            `[EXCHANGE_SL_ENGINE][EPOCH_FENCED] [${this.config.symbol}:${side}] Worker Epoch #${currentEpoch} evicted after order placement. Aborting.`
          );
          return;
        }
        ...
        if (placedOrderId) {
          ...
          console.error(
            `[EXCHANGE_SL_ENGINE][PLACEMENT_FAILED] [${this.config.symbol}:${side}] Failed to place replacement SL @ $${currentTargetPrice}.`
          );
        }
        ...
        if (this.pendingSlSyncTargets.has(lockKey)) {
          ...
          const tickSize = SymbolPrecisionRegistry.getTickSize(this.config.symbol);
    ...
    if (longSummary.isOccupied && longSummary.totalQuantity > 0) {
      ...
      if (!isLockInFlight) {
        ...
        if (!audit.isProtected) {
          ...
          this.syncExchangeStopLossOrder("LONG", longSummary.totalQuantity, longSummary.stopLossPrice).catch((err: unknown) => {
    ...
    if (shortSummary.isOccupied && shortSummary.totalQuantity > 0) {
      ...
      if (!isShortLockInFlight) {
        ...
        if (!audit.isProtected) {
          ...
          this.syncExchangeStopLossOrder("SHORT", shortSummary.totalQuantity, shortSummary.stopLossPrice).catch((err: unknown) => {
  ...
  public evaluateTick(): StrategySignalResult {
    ...
    try {
      ...
      const effectiveStalenessCeiling = Math.min(
        this.stalenessMaxCapMs,
        Math.max(this.stalenessMinFloorMs, this.rollingPacketAgeMs + this.stalenessJitterMultiplier * this.rollingPacketJitterMs)
      );
      ...
      if (!isTickValid || isStale || currentSpread > maxSpreadAllowed) {
        ...
        if (currentSpread > maxSpreadAllowed || !isTickValid) {
          this.annihilateRestingEntryOrders(
            !isTickValid ? "INVALID_TICK_PRICES" : `SPREAD_BLOWOUT_${currentSpread.toFixed(4)}_GT_${maxSpreadAllowed.toFixed(4)}`
          );
        }
      ...
      if (markPrice > 0) {
        ...
        if (activeTriggers.length > 0) {
          ...
          const exitOrderType: "LIMIT" | "MARKET" = isPassiveMakerExit ? "LIMIT" : "MARKET";
          const exitTimeInForce: "GTC" | "IOC" | "GTX" | undefined = isPassiveMakerExit ? "GTX" : undefined;
          ...
          const formattedExitPrice = SymbolPrecisionRegistry.formatPrice(this.config.symbol, rawTargetExitPrice);
          ...
          if (riskResult.passed) {
            ...
            executionPromise = (async () => {
              if (allCancelOrderIds.length > 0) {
                console.log(
                  `[MAKER_TP_ENGINE][EMERGENCY_CANCEL] Cancelling ${allCancelOrderIds.length} open POST_ONLY limit TP orders before exit dispatch...`
                );
                try {
              ...
              const exitCid = ClientOrderIdGenerator.generate(this.config.symbol, primaryTrigger.slotId, isPassiveMakerExit ? "MK" : "EM");
      ...
      const hasActiveShortSlot = this.hedgeLedger.getShortSlots().some(
        (s) => s.isOccupied || s.lifecycleState === "PENDING_ENTRY"
      );
      ...
      if (isPortfolioCapacityReached && !hasExistingPositionForThisSymbol) {
        if ((isBuySignal || isSellSignal) && seq % 1000n === 0n) {
          console.warn(
            `[PORTFOLIO_CAPACITY_LOCK][BLOCKED] [${this.config.symbol}] Portfolio hard cap reached (${globalActivePositions}/${this.client.maxAssets} active positions). Entry blocked.`
          );
        }
        isBuySignal = false;
        isSellSignal = false;
      ...
      if (isShortActiveOrPending && isBuySignal) {
        if (seq % 1000n === 0n) {
          console.warn(
            `[UNIDIRECTIONAL_MUTEX][BLOCKED] [${this.config.symbol}] BUY signal rejected: Active/Pending SHORT position exists on this asset.`
          );
        }
        isBuySignal = false;
      }
      ...
      if (isLongActiveOrPending && isSellSignal) {
        if (seq % 1000n === 0n) {
          console.warn(
            `[UNIDIRECTIONAL_MUTEX][BLOCKED] [${this.config.symbol}] SELL signal rejected: Active/Pending LONG position exists on this asset.`
          );
        }
        isSellSignal = false;
      }
      ...
      if (riskResult.passed) {
        ...
        const preFlightKey = -Math.abs(timeSynchronizer.getAdjustedNowMs());
        ...
        if (targetPosSide === "SHORT") {
          this.client.setShortCooldownLock(timeSynchronizer.getAdjustedNowMs() + this.config.cooldownMs, this.assetIndex);
          this.client.setLastShortFillPrice(this.reusableOrderIntent.price, this.assetIndex);
        } else if (targetPosSide === "LONG") {
          this.client.setLongCooldownLock(timeSynchronizer.getAdjustedNowMs() + this.config.cooldownMs, this.assetIndex);
        ...
        executionPromise = (async () => {
          ...
          if (!isPfValid || pfSpread > maxPfSpread || pfSpreadBps > 5.0) {
            console.warn(
              `[PREFLIGHT_SPREAD_BARRIER][BLOCKED] [${this.config.symbol}:${entrySlotId}] Pre-flight spread blowout intercepted before socket write! Spread: $${pfSpread.toFixed(4)} (${pfSpreadBps.toFixed(1)} bps) > Ceiling: $${maxPfSpread.toFixed(4)}. Aborting order dispatch.`
            );
  ...
  public getRollingPacketAgeMs(): number {
    return this.rollingPacketAgeMs;
  }

  public getRollingPacketJitterMs(): number {
    return this.rollingPacketJitterMs;
  }

  public getEffectiveStalenessCeiling(): number {
    return Math.min(
      this.stalenessMaxCapMs,
      Math.max(this.stalenessMinFloorMs, this.rollingPacketAgeMs + this.stalenessJitterMultiplier * this.rollingPacketJitterMs)
    );
  ...
  public getHazardEngine(): MicrostructureHazardEngine {
