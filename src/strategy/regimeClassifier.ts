...
export type MarketRegimeState = "MEAN_REVERT" | "RANDOM_WALK" | "TRENDING";

export class OnlineVarianceRatioClassifier {
  public readonly k: number;
  public readonly windowSize: number;
  public readonly alpha: number;

  // Zero-GC fixed ring buffer for price history
  private readonly priceRing: Float64Array;
  private head: number = 0;
  private count: number = 0;

  // Online EWMA statistics for 1-period and k-period log-returns
  private ewmaMean1: number = 0.0;
  private ewmaVar1: number = 0.0;
  private ewmaMeanK: number = 0.0;
  private ewmaVarK: number = 0.0;

  // Last computed variance ratio
  private cachedVR: number = 1.0;

  constructor(k: number = 5, windowSize: number = 60, alpha: number = 0.02) {
    if (k < 2) {
   ...
   */
  public updatePrice(midPrice: number): void {
   ...
   */
  public getVarianceRatio(): number {
   ...
   */
  public getRegimeState(): MarketRegimeState {
  ...
  public getEwmaVar1(): number {
    return this.ewmaVar1;
  }

  public getEwmaVarK(): number {
