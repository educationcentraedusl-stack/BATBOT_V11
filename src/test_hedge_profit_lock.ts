...
import { RiskGuard, RISK_PASSED } from "./strategy/risk";
...
async function runHedgeAndProfitLockTests() {
  ...
  const hedgeLedger = new HedgePositionLedger("BTCUSDT", maxSlots);
  ...
  const riskGuard = new RiskGuard({ dailyProfitLockTargetUsdt: profitLock });
