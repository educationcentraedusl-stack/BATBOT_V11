...
export class ClientOrderIdGenerator {
  private static readonly SYSTEM_PREFIX = "BB11";
  private static nonceCounter = 0;

  private static readonly COMPACT_SYMBOL_MAP: Record<string, string> = {
    BTCUSDT: "BTC",
    ETHUSDT: "ETH",
    SOLUSDT: "SOL",
    BNBUSDT: "BNB",
  ...
  private static compactSlotId(slotId: string): string {
    if (slotId === "CORE_LONG") return "L0";
  ...
  private static expandSlotId(compactSlot: string): string {
    if (compactSlot === "L0") return "CORE_LONG";
  ...
  public static generate(
    symbol: string,
    slotId: string,
    orderType: "EN" | "TP1" | "TP2" | "TP3" | "TP4" | "TP5" | "SL" | "TS" | "EM" | string
  ): string {
   ...
   */
  public static parse(clientOrderId?: string | null): ParsedClientOrderId | null {
